...
class ElastAlerter():
    ...
    def get_query(filters, starttime=None, endtime=None, sort=True, timestamp_field='@timestamp', to_ts_func=dt_to_ts, desc=False,
        ...
        #zyl add
        es_filters = copy.deepcopy(filters)
        if starttime and endtime:
            if 'must' in es_filters:
                es_filters['must'].insert(0, {'range': {timestamp_field: {'gt': starttime, 'lte': endtime}}})
            else:
                es_filters['must'] = [{'range': {timestamp_field: {'gt': starttime, 'lte': endtime}}}]
