    ...
    def _add_custom_alert_text(self):
        ...
        if 'alert_text_args' in self.rule:
            ...
            for name, kw_name in self.rule.get('alert_text_kw').items():
                ...
                #zyl add
                if name == "filter":
                    val = repr(val).decode('unicode_escape')
                #zyl add
        ...
        #zyl add
        patrol_title = unicode(self.rule.get('patrol_title', ''))
        if 'patrol_title_kw' in self.rule:
            kw = {}
            for name, kw_name in self.rule.get('patrol_title_kw').items():
                val = lookup_es_key(self.match, name)
                if val is None:
                    val = self.rule.get(name)
                if name == "@timestamp":
                    val = pretty_ts(val)
                kw[kw_name] = missing if val is None else val
            patrol_title = patrol_title.format(**kw)

        patrol_text = unicode(self.rule.get('patrol_text', ''))
        if 'patrol_text_kw' in self.rule:
            kw = {}
            for name, kw_name in self.rule.get('patrol_text_kw').items():
                val = lookup_es_key(self.match, name)
                if val is None:
                    val = self.rule.get(name)
                if name == "@timestamp":
                ...
                if name == "filter":
                    val = repr(val).decode('unicode_escape')
                kw[kw_name] = missing if val is None else val
            patrol_text = patrol_text.format(**kw)

            #############生产地址#############
            url ='http://x.x.x.x:50368/axis/services/AlarmSwitchService?wsdl'
            ...
            patrol_time = kw['time']
            xmlContent = xml_convert(kw['system_no'], 'ELK', 'ELK', 'ELK', 'major', patrol_text, patrol_time, kw['_id'], 'ELK', patrol_title)

            elastalert_logger.info('xmlContent=%s' % (xmlContent))
            #zyl need to open
            #patrol_result=client.service.transferAlarm('','','','',xmlContent.decode('UTF-8'))
            patrol_result=""
            #zyl need to open

        patrollogpath = unicode(self.rule.get('patrollogpath', ''))
        if 'patrollogpath_kw' in self.rule:
            kw = {}
            for name, kw_name in self.rule.get('patrollogpath_kw').items():
                val = lookup_es_key(self.match, name)
                if val is None:
                    val = self.rule.get(name)
                if name == "@timestamp":
                    val = pretty_ts(val)
                kw[kw_name] = missing if val is None else val
            patrollogpath = patrollogpath.format(**kw)
        if not os.path.exists(os.path.dirname(patrollogpath)):
            try:
                os.makedirs(os.path.dirname(patrollogpath))
            except OSError as exc:
                if exc.errno != errno.EEXIST:
                    raise EAException('Can not create patrol log directory')
        out_content = patrol_time + ' ' + patrol_title + u'\t发送内容:' + patrol_text + '\t' + u'调用结果:\n' + patrol_result + '\n'
        with open(patrollogpath, "a") as output_file:
            output_file.write(out_content.encode('UTF-8'))
        #zyl add
