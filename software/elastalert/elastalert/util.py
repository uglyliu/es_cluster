...
#zyl add
def xml_convert(host,instance,parameter,Class,status,value,occurtime,alarmId,appSystem,alarmTitle):
    opDetail={'host':host,'instance':instance,'parameter':parameter,'class':Class,'status':status,\
                            'value':value,'max':'','min':'','occurtime':occurtime,'alarmId':alarmId,'appSystem':appSystem,'alarmTitle':alarmTitle}
    xmlContent=[]
    for k in sorted(opDetail.keys()):
        v = opDetail.get(k).encode('UTF-8')
        xmlContent.append('<fieldInfo><fieldEnName>{key}</fieldEnName><fieldContent>{value}</fieldContent></fieldInfo>'.format(key=k, value=v))
    return '<?xml version="1.0" encoding="gb2312"?><opDetail><recordInfo>{}</recordInfo></opDetail>'.format(''.join(xmlContent))
#zyl add
